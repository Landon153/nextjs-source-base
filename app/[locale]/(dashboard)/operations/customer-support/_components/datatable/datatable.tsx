...
import { columns } from '@/app/[locale]/(dashboard)/operations/customer-support/_components/datatable/columns';
...
interface DatatableProps extends HTMLAttributes<HTMLDivElement> {
  data: PaginatedResponse<CustomerSupport>;
  hasFilter: boolean;
    ...
    <Card className={cn('min-w-0', className)} {...props}>
      ...
      <CardContent className="border-t p-0">
        <TableContent
          ...
          onRowClick={(row) => {
            router.push(
              row.original.isAnswer
                ? `/operations/customer-support/${row.original.id}`
                : `/operations/customer-support/${row.original.id}/edit`,
            );
          }}
          onRowHover={(row) => {
            router.prefetch(
