...
import { columns } from '@/app/[locale]/(dashboard)/operations/notices/_components/datatable/columns';
import { DeleteRowsDialog } from '@/app/[locale]/(dashboard)/operations/notices/_components/delete-rows-dialog';
    ...
    <Card className={cn('min-w-0', className)} {...props}>
      ...
      <CardContent className="border-t p-0">
        <TableContent
          ...
          table={table}
          onRowClick={(row) => {
            router.push(`/operations/notices/${row.original.id}/edit`);
          }}
          onRowHover={(row) => {
            router.prefetch(`/operations/notices/${row.original.id}/edit`);
          }}
        />
      </CardContent>
