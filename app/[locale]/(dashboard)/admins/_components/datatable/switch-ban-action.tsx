...
import { toggleUserSuspension } from '@/lib/services/default/features/user';
import { type Admin } from '@/lib/services/default/types/user';

export function SwitchBanAction({ row }: { row: Row<Admin> }): JSX.Element {
  ...
  return (
    <Switch
      checked={row.getValue<UserStatus>('status') === UserStatus.Banned}
      disabled={isPending}
      onCheckedChange={() => {
        mutate({
          userId: row.original.id,
          payload: {
            status: row.original.status === UserStatus.Banned ? UserStatus.Active : UserStatus.Banned,
            reasonBanned: '',
          },
        });
      }}
