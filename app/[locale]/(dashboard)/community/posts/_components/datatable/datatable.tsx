...
import { columns } from '@/app/[locale]/(dashboard)/community/posts/_components/datatable/columns';
    ...
    <Card className={cn('min-w-0', className)} {...props}>
      ...
      <CardContent className="border-t p-0">
        <TableContent
          ...
          table={table}
          onRowClick={(row) => {
            router.push(`/community/posts/${row.original.id}`);
          }}
          onRowHover={(row) => {
            router.prefetch(`/community/posts/${row.original.id}`);
          }}
        />
      </CardContent>
