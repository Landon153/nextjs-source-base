...
import { columns } from '@/app/[locale]/(dashboard)/contents/events/_components/datatable/columns';
import { DeleteRowsDialog } from '@/app/[locale]/(dashboard)/contents/events/_components/delete-rows-dialog';
    ...
    <Card className={cn('', className)} {...props}>
      <CardHeader className="min-h-15 justify-center py-3">
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
          <div className="flex items-center gap-5">
            <CardTitle className="text-sm font-bold">전체 {data.items.length}</CardTitle>
      ...
      <CardContent className="border-t p-0">
        <TableContent
          ...
          table={table}
          onRowClick={(row) => {
            router.push(`/contents/events/${row.original.id}/edit`);
          }}
          onRowHover={(row) => {
            router.prefetch(`/contents/events/${row.original.id}/edit`);
          }}
        />
      </CardContent>
