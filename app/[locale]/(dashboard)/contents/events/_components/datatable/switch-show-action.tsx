...
import { toggleShowEvent } from '@/lib/services/default/features/events';
import { type Event } from '@/lib/services/default/types/event';

export function SwitchShowAction({ row }: { row: Row<Event> }): JSX.Element {
  const queryClient = getQueryClient();
  const router = useRouter();

  const { mutate, isPending } = useMutation({
    mutationFn: toggleShowEvent,
