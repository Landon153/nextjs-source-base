...
import { columns } from '@/app/[locale]/(dashboard)/reports/blocks/_components/datatable/columns';
    ...
    <Card className={cn('min-w-0', className)} {...props}>
      ...
      <CardContent className="border-t p-0">
        <TableContent
          ...
          table={table}
          onRowClick={(row) => {
            router.push(`/reports/blocks/${row.original.id}`);
          }}
          onRowHover={(row) => {
            router.prefetch(`/reports/blocks/${row.original.id}`);
          }}
        />
      </CardContent>
